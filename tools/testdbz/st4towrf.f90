...
!--------------------------------------------------------!
  ...
  inquire(file='./st4obs.nc',exist=alive2)  
    ...
    do i=1,nx
      ...
      do jj=jos,joe  !1,st4dim(1,2)
        do ii=ios,ioe  !1,st4dim(1,1)
          if(abs(var3d(2)%val(i,j,1)-st4info(2)%val(ii,jj,1))>0.1.or.  &
             abs(var3d(1)%val(i,j,1)-st4info(3)%val(ii,jj,1))>0.1.or.  &
			 st4info(1)%val(ii,jj,1)>1.e3) then
            cycle
          endif
		  zp=0.0
