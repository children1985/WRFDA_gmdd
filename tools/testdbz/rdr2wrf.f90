...
!--------------------------------------------------------!
  ...
  print*,"read in parameter",trim(argc)
  if(trim(argc)=="") then
    print*,"skipn is set to default value: 1"
    skipn=1
  else
    print*,"skipn is read in"
    read(argc,*) skipn
  endif
  print*,"skipn value of ",skipn," is used"
...
!--------------------------------------------------------!
   ...
   rfm=0
   call intpl_obs2wrf(rfm,dim(i,1),dim(i,2),dim(i,3),skipn)
...
!/'XLONG','XLAT','PH','PHB','T','P','PB','QRAIN','QSNOW','QGRAUP'/
  ...
  do j=1,dim(nvar,2)
    do i=1,dim(nvar,1)
       ...
       !if(maxrfcol>=-99.0) then
        !print*,"processing ",i,j,maxrfcol    
        do k=1,dim(nvar,3)
          !if(rfm(i,j,k)<-99.0) cycle
          rfm(i,j,k)=min(55.0,rfm(i,j,k))
      ...
      do i=1,nx
        ...
        do while(n<=ndata)
          ...
          do kk=1,rdrdata(n)%nlev
            ...
            if(dis_closest>d) then
              dis_closest=d
              val_closest=rdrdata(n)%rf(kk)
            endif
